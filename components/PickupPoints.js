...
import { selectPickupPoints, setDestination } from '../slices/navSlice';
    ...
    return (
        <FlatList
            ...
            (
                <TouchableOpacity
                    onPress={() => {
                        dispatch(setDestination({ description: item.name, location: { lat: item.lat, lng: item.lng } }))
                    ...
                    />
                    <View>
                        <Text style={tw`font-semibold text-lg ${selected == item.id && 'text-blue-500'}`}>{item.city}</Text>
                        <Text style={tw`text-gray-500`}>{item.name}</Text>
                    </View>
                </TouchableOpacity>
            )}
        />
